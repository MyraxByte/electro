// Auto-generated by ElectroJS codegen. Do not edit.
// @ts-nocheck
// ElectroJS renderer bridge contract types — provides IDE completions for bridge access and forwarded signals.

export {};

type _Instance<T> = T extends abstract new (...args: never[]) => infer R ? R : never;
type _SignalPayloadFromMethodParam<T, K extends PropertyKey, I extends number> =
    K extends keyof _Instance<T>
        ? _Instance<T>[K] extends (...args: infer A) => infer _Ignored
            ? I extends keyof A
                ? A[I]
                : void
            : never
        : never;
type _SignalPayloadFromMethod<T, K extends PropertyKey> =
    _SignalPayloadFromMethodParam<T, K, 0>;
type _BridgeInputFromMethod<T, K extends PropertyKey> =
    K extends keyof _Instance<T>
        ? _Instance<T>[K] extends (...args: infer A) => infer _Ignored
            ? A extends []
                ? undefined
                : A extends [infer TOnly]
                    ? TOnly
                    : A
            : never
        : never;
type _BridgeOutputFromMethod<T, K extends PropertyKey> =
    K extends keyof _Instance<T>
        ? _Instance<T>[K] extends (...args: infer _Args) => infer V
            ? Awaited<V>
            : never
        : never;

declare module "@electrojs/renderer" {
    interface BridgeQueries {
        "app:getShellState": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "getShellState">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "getShellState">>;
        "notes:getNotes": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/notes/notes.service").NotesService, "getNotes">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/notes/notes.service").NotesService, "getNotes">>;
        "app:minimizeWindow": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "minimizeWindow">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "minimizeWindow">>;
        "app:toggleMaximizeWindow": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "toggleMaximizeWindow">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "toggleMaximizeWindow">>;
        "app:closeWindow": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "closeWindow">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "closeWindow">>;
        "app:openDocumentation": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "openDocumentation">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "openDocumentation">>;
        "notes:createNote": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/notes/notes.service").NotesService, "createNote">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/notes/notes.service").NotesService, "createNote">>;
        "notes:deleteNote": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/notes/notes.service").NotesService, "deleteNote">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/notes/notes.service").NotesService, "deleteNote">>;
    }

    interface BridgeCommands {
        "app:minimizeWindow": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "minimizeWindow">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "minimizeWindow">>;
        "app:toggleMaximizeWindow": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "toggleMaximizeWindow">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "toggleMaximizeWindow">>;
        "app:closeWindow": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "closeWindow">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "closeWindow">>;
        "app:openDocumentation": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "openDocumentation">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/app.shell.service").ShellService, "openDocumentation">>;
        "notes:createNote": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/notes/notes.service").NotesService, "createNote">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/notes/notes.service").NotesService, "createNote">>;
        "notes:deleteNote": import("@electrojs/renderer").BridgeContractEntry<_BridgeInputFromMethod<typeof import("../../runtime/src/modules/notes/notes.service").NotesService, "deleteNote">, _BridgeOutputFromMethod<typeof import("../../runtime/src/modules/notes/notes.service").NotesService, "deleteNote">>;
    }

    interface BridgeSignals {

    }
}
