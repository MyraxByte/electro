...
Views support the full lifecycle. See [Application Lifecycle](/core/lifecycle).
...
Each view package gets a generated `electro-env.d.ts` that augments `@electrojs/renderer` for that specific view. Include it in the package `tsconfig.json`. See [Code Generation](/advanced/codegen) for details.
