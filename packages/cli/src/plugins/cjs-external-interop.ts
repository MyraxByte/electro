...
/**
 * Rewrites named and namespace imports from external CommonJS deps to runtime-safe accessors.
 *
 * Example:
 *   import * as electronUpdater from "electron-updater"
 * becomes
 *   import * as __cjs_ext_0__ from "electron-updater"
 *   const electronUpdater = __cjs_ext_0__.default ?? __cjs_ext_0__
 *
        ...
        renderChunk(code, _chunk, { format, sourcemap }) {
            ...
            for (const match of code.matchAll(STATIC_IMPORT_RE)) {
                ...
                if (parsed.namespaceImport) {
                    lines.push(`const ${parsed.namespaceImport} = ${ns}.default ?? ${ns};`);
                }
...
function isCjsExternalSpecifier(specifier: string, cjsDepSet: Set<string>): boolean {
    if (specifier.startsWith(".") || specifier.startsWith("/") || specifier.startsWith("\0") || specifier.startsWith("node:")) {
        return false;
    }
...
function parseImportClause(clause: string): ParsedImport | null {
    const namespaceOnly = /^(\s*type\s+)?\*\s+as\s+([A-Za-z_$][\w$]*)\s*$/.exec(clause);
    if (namespaceOnly) {
        const namespaceImport = namespaceOnly[2];
        if (!namespaceImport) return null;
        return { namespaceImport, named: [] };
    }

    const defaultAndNamespace = /^([A-Za-z_$][\w$]*)\s*,\s*(?:type\s+)?\*\s+as\s+([A-Za-z_$][\w$]*)\s*$/.exec(clause);
    if (defaultAndNamespace) {
        const defaultImport = defaultAndNamespace[1];
        const namespaceImport = defaultAndNamespace[2];
        if (!defaultImport || !namespaceImport) return null;
    ...
    if (!defaultImport && named.length === 0) return null;
