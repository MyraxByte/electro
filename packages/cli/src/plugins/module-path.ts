    ...
    return {
        ...
        async load(id): Promise<string | undefined> {
            ...
            if (!mainChunk || mainChunk.type !== "chunk") {
                throw new Error(`Expected a main output chunk for "${file}"`);
            }
        ...
        renderChunk(code, chunk, opts) {
            ...
            while (match) {
                ...
                if (!hash) {
                    throw new Error("Missing emitted module-path placeholder hash");
                }
                const filename = this.getFileName(hash);
                if (!filename) {
                    throw new Error(`Missing emitted module-path filename for placeholder "${hash}"`);
                }
...
async function bundleModulePath(input: string, config: InlineConfig, watch: boolean): Promise<{ output: SubBuildOutput[]; watchFiles: string[] }> {
        ...
        plugins: [
            {
                ...
                outputOptions(output) {
                    if (typeof output.entryFileNames !== "function" && output.entryFileNames) {
                        return {
                            ...output,
                            entryFileNames: `[name]-[hash]${extname(output.entryFileNames)}`,
                        };
                    }
                },
            } satisfies Plugin,
