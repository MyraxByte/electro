    ...
    return {
        ...
        async load(id) {
            ...
            if (ASSET_QUERY_RE.test(id)) {
                return [`import { join } from "node:path"`, `export default join(import.meta.dirname, ${referenceId})`].join("\n");
            }
        ...
        renderChunk(code, chunk) {
            ...
            while (match) {
                ...
                if (!hash) {
                    throw new Error("Missing emitted asset placeholder hash");
                }
                const filename = this.getFileName(hash);
                if (!filename) {
                    throw new Error(`Missing emitted asset filename for placeholder "${hash}"`);
                }
