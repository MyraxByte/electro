...
import { type CliViewDefinition, getViewRoot, getViteViewEntry, type ElectroConfigLike } from "./dev/views";
 ...
 */
export function validateConfig(config: ElectroConfigLike): void {
...
export function validateViews(views: readonly CliViewDefinition[]): void {
    // Duplicate view ids
    const ids = new Set<string>();
    for (const view of views) {
        if (ids.has(view.id)) {
            error(`Duplicate view id "${view.id}". View ids must be unique.`);
            process.exit(1);
        }
        ids.add(view.id);
    }

    // View config and entry files must exist
    for (const view of views) {
        if (!existsSync(view.__source)) {
            error(`View "${view.id}" config source not found: ${view.__source}`);
            process.exit(1);
        }

        const viewDir = getViewRoot(view);
        const viewEntry = resolve(viewDir, getViteViewEntry(view));
        if (!existsSync(viewEntry)) {
            error(`View "${view.id}" entry not found: ${viewEntry}`);
            process.exit(1);
        }
    }
}
...
export function validateSourcemap(value: string): void {
    if (!VALID_SOURCEMAP_VALUES.includes(value)) {
        warn(`Unknown --sourcemap value "${value}". ` + `Valid values: ${VALID_SOURCEMAP_VALUES.join(", ")}. Defaulting to "linked".`);
    }
}
 ...
 */
export function validateMergedNodeConfig(config: { build?: Record<string, unknown>; ssr?: unknown }, scope: string): void {
    ...
    if (config.ssr === false || config.build?.ssr === false) {
        throw new Error(`[electro] ${scope}: SSR cannot be disabled in Node scopes. ` + "SSR mode is required for correct Node.js module resolution.");
    }
}
