...
export async function resolveExternals(root: string): Promise<ResolvedExternals> {
    ...
    const deps = new Set<string>(["electron", ...Object.keys(pkg.dependencies ?? {}), ...Object.keys(pkg.optionalDependencies ?? {})]);
    ...
    const interopDepsArray = [...interopDeps];
    const cjsInteropDeps = (
        await Promise.all(
            interopDepsArray.map(async (dep) => ({
                dep,
...
async function isLikelyCommonJsDependency(root: string, dep: string): Promise<boolean> {
    ...
    try {
        ...
        // If metadata is unavailable (workspace hoisting edge cases, optional dep),
