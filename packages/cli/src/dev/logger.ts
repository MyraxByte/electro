...
export function footer(message: string, endpoints?: string | readonly FooterEndpoint[]): void {
    if (levels[currentLevel] > levels.info) return;
    console.log(`\n  ${bold}${green}✓ ${message}${reset}`);
    if (typeof endpoints === "string") {
        console.log(`    ${yellow}→ ${endpoints}${reset}`);
    } else {
        for (const endpoint of endpoints ?? []) {
            console.log(`    ${yellow}${endpoint.label}${reset} ${dim}→${reset} ${endpoint.url}`);
        }
    }
    console.log("");
}
...
export function logSession(meta: SessionMeta): void {
    ...
    const viewEntry =
        views.length === 0 ? `${dim}(none)${reset}` : views.length === 1 ? toProjectRelative(meta.root, views[0]!.entry) : `${views.length} configured`;
    ...
    const rows = [
        ...
        {
            scope: `${green}${views.length > 1 ? "views" : "view"}${reset}`,
            entry: viewEntry,
    ...
    if (meta.views && meta.views.length > 0) {
        ...
        const viewIdWidth = Math.max(2, ...meta.views.map((v) => v.id.length)) + 2;

        for (const view of meta.views) {
            const entry = toProjectRelative(meta.root, view.entry);
            console.log(`  ${padVisible(view.id, viewIdWidth)}${dim}${entry}${reset}`);
        }
    }
